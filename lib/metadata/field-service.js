...
async function validateTypeSpecifics(
  ctx,
  { type, options, relation, settings, objectMetadataId },
  session,
) {
  ...
  if (type === 'ROLLUP') {
    ...
    if (
      !relField ||
      String(relField.relation?.targetObjectMetadataId) !== String(objectMetadataId)
    ) {
    ...
    if (cfg.operation !== 'count') {
      ...
      if (!aggField) {
        throw new ValidationError(
          `El campo "${cfg.aggregateFieldName}" no existe en el objeto de origen`,
        );
      }
