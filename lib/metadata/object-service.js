...
export async function createObject(ctx, input) {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      // Campo de sistema "Creado por" (ACTOR): su valor vive en la raíz del
      // registro (`records.createdBy`). Se crea directo (createField no admite
      // campos de sistema ni el nombre reservado `createdBy`). Así los registros
      // del objeto muestran quién los creó, como en los objetos estándar.
