...
export async function saveInviteStep(ctx, { emails }) {
  ...
  for (const email of unique) {
    try {
      // Excepción deliberada al requisito de email confirmado: este paso ocurre
      // justo tras el alta, así que exigirlo dejaría el paso inútil para todo el
      // mundo. El riesgo queda acotado por el tope de 3 direcciones de arriba y
      // por el freno de altas por IP (`lib/auth/throttle.js`).
