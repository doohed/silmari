 ...
 *
 * La comparación exige el separador: un `startsWith(BASE)` a secas daría por
 * buena `/app/storage-otro/…` cuando BASE es `/app/storage`. Hoy no es
 * explotable —la key la genera el servidor y al leer viene de la BD—, pero una
 * guardia de ruta que se puede saltar con un carácter no es una guardia.
 */
function pathFor(key) {
  const full = resolve(join(BASE, key));
  if (full !== BASE && !full.startsWith(BASE + sep)) {
    throw new Error('Clave de storage no válida');
  }
  return full;
}
