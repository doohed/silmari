...
export async function throttleAuth(flow, { email } = {}) {
  ...
  try {
    ...
    if (err instanceof TooManyRequestsError) {
      // Un freno disparado es la primera señal de que alguien está probando
      // combinaciones; sin registrarlo no hay forma de verlo venir.
      logSecurityEvent('auth.throttled', { flow, email, ip });
