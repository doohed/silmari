...
export const getContext = cache(async () => {
  ...
  // Las dos consultas van en paralelo: es la ruta caliente de cada petición.
  const [member, user] = await Promise.all([
    WorkspaceMember.findOne({
      workspaceId: session.workspaceId,
      userId: session.userId,
    }).lean(),
    User.findOne({ _id: session.userId }).select('sessionsValidFrom').lean(),
  ]);

  if (!member || !user) return null;
  // Una sesión firmada antes del último cambio de contraseña ya no vale.
  if (!isSessionCurrent(session, user.sessionsValidFrom)) return null;
