...
export async function syncFieldIndex(field) {
  ...
  // Si el índice ya existe con otra forma (migración de esquema), recréalo. En
  // una BD nueva la colección `records` puede no existir todavía (p. ej. el
  // primer alta, antes del primer registro): sin colección no hay índices
  // previos, así que tratamos el listado como vacío y seguimos a `createIndex`,
  // que la crea.
