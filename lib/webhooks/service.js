...
export async function createWebhook(ctx, { targetUrl, operations }) {
  ...
  try {
    ...
    // Un intento de apuntar hacia dentro no es un error de usuario cualquiera:
    // es la señal de que alguien está tanteando la red desde nuestro servidor.
    logSecurityEvent('webhook.blocked', {
      workspaceId: String(ctx.workspaceId),
      userId: String(ctx.userId),
      targetUrl: String(targetUrl).slice(0, 200),
    });
    throw err;
  }
  ...
  logSecurityEvent('webhook.created', {
    workspaceId: String(ctx.workspaceId),
    userId: String(ctx.userId),
