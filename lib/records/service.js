...
export async function createRecord(ctx, { objectSlug, data, source = 'MANUAL' }) {
  ...
  // Los campos MEMBER sin valor se rellenan con el creador (si es un usuario
  // real, no una API key). Editable después.
  if (!String(ctx.userId).startsWith('apikey:')) {
  ...
  dispatchWebhooks(ctx, { operation: `${object.nameSingular}.created`, payload: result }).catch(
    () => {},
  );
  return result;
}
...
export async function listRecords(
  ctx,
  { objectSlug, filters, sorts, cursor, limit, includeDeleted },
) {
  assertTenant(ctx);
  const object = await resolveObject(ctx, objectSlug);
  await connectToDatabase();
  ...
  const docs = await Record.find(match)
    .sort(sort)
    .limit(take + 1)
    .lean();
...
export async function updateRecord(ctx, { objectSlug, recordId, data }) {
  ...
  dispatchWebhooks(ctx, { operation: `${object.nameSingular}.updated`, payload: result }).catch(
    () => {},
  );
  return result;
}
...
export async function softDeleteRecord(ctx, { objectSlug, recordId }) {
  ...
  dispatchWebhooks(ctx, {
    operation: `${object.nameSingular}.deleted`,
    payload: { id: String(record._id) },
  }).catch(() => {});
}
