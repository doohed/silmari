...
import { aggregateRollup } from '@/lib/field-types/rollup-eval';
...
function computeFormulas(base, fields, allFields) {
  const formulaFields = fields
    ...
    .map((f) => ({
      name: f.name,
      formula: f.settings.formula,
      deps: formulaDependencies(f.settings.formula),
    }));
...
/**
 * Calcula los campos ROLLUP (solo lectura) agregando un campo de los registros
 * relacionados que apuntan a cada registro base vía una relación entrante. Un
 * ROLLUP se configura con `settings.rollup = { relationFieldId, operation,
 * aggregateFieldName? }`; `relationFieldId` es el campo RELATION (MANY_TO_ONE) de
 * OTRO objeto cuyo destino es este. Recorre `recordRelations` (espejo del inverso)
 * y excluye los registros origen borrados (soft delete).
 */
async function computeRollups(ctx, { base, rollupFields }) {
  if (rollupFields.length === 0) return;
  const baseIds = base.map((r) => r.id);

  for (const rf of rollupFields) {
    const cfg = rf.settings.rollup;
    const relField = await FieldMetadata.findOne({
      _id: cfg.relationFieldId,
      workspaceId: ctx.workspaceId,
    }).lean();
    if (!relField || relField.type !== 'RELATION') {
      for (const r of base) r.data[rf.name] = null;
      continue;
    }

    // Aristas origen→base de esa relación, acotadas a los registros base.
    const edges = await RecordRelation.find({
      workspaceId: ctx.workspaceId,
      fieldMetadataId: relField._id,
      targetRecordId: { $in: baseIds },
    })
      .select('sourceRecordId targetRecordId')
      .lean();

    // Registros origen vivos (con el campo a agregar).
    ...
    // Metadata del campo a agregar (para leer CURRENCY.amount, etc.).
    const aggFieldMeta =
      cfg.operation !== 'count' && cfg.aggregateFieldName
        ? await FieldMetadata.findOne({
            workspaceId: ctx.workspaceId,
    ...
    const valuesByTarget = new Map(baseIds.map((id) => [id, []]));
    for (const e of edges) {
      const src = sourceById.get(String(e.sourceRecordId));
      if (!src) continue; // origen borrado → no cuenta
      const bucket = valuesByTarget.get(String(e.targetRecordId));
      if (!bucket) continue;
      bucket.push(numericValue(aggFieldMeta, src.data?.[cfg.aggregateFieldName]));
    }

    for (const r of base) {
      r.data[rf.name] = aggregateRollup(cfg.operation, valuesByTarget.get(r.id) ?? []);
    }
  }
}
...
export async function hydrateRecords(ctx, { records, fields, allFields }) {
  ...
  const rollupFields = fields.filter((f) => f.type === 'ROLLUP' && f.settings?.rollup);
  ...
  await computeRollups(ctx, { base, rollupFields });
