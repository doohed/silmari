...
export function buildQuery({
  ...
  for (const filter of filters) {
    ...
    // El valor viene del cliente: se recorta a un escalar (o lista de escalares)
    // antes de tocar el match, para que no pueda colar un operador de Mongo.
    const value = coerceFilterValue(filter.value, filter.operator);
    and.push(def.buildFilter(field.name, filter.operator, value, field));
  }
  ...
  if (primaryField && !isSortableField(primaryField)) {
    throw new ValidationError(`No se puede ordenar por el campo "${primarySort.fieldName}"`);
  }
  ...
  if (decoded) {
    // Un id que no es un ObjectId hace que el constructor lance, y ese error no
    // es de dominio: saldría como 500. Un cursor manipulado es entrada no
    // válida, no un fallo del servidor.
...
export function buildNextCursor(lastRecord, primaryField) {
  ...
  // Red de seguridad: si el valor de orden no es escalar, `decodeCursor` va a
  // tirar este cursor por la borda y la siguiente página repetirá la primera
  // —filas duplicadas al hacer scroll, sin un solo error en el log—. Vale más
  // reventar aquí, donde el mensaje señala al tipo que le falta un `sortPath`.
  if (sortValue !== null && typeof sortValue === 'object') {
    throw new Error(
      `El campo "${primaryField?.name}" (${primaryField?.type}) produce un valor de orden no escalar: ` +
        'necesita un `sortPath` a una hoja escalar, o `isSortable: false`.',
    );
  }
