...
const formula = {
  type: 'FORMULA',
  isSortable: false, // no se persiste: se calcula al hidratar
  // Campo calculado: no se persiste en `data`; su valor lo computa la
  // hidratación a partir de otros campos (settings.formula). Solo lectura.
  schema: (m) => nullable(z.number(), m),
  defaultValue: () => null,
  normalize: () => undefined, // nunca se guarda desde el cliente
  isReadOnly: true,
  isComputed: true,
  // No filtrable ni ordenable en BD (no existe el valor almacenado); la tabla lo
  // muestra pero no ofrece filtro. `compare` sirve para ordenación en memoria.
  filterOperators: [],
  buildFilter: () => ({}),
  compare: defaultCompare,
  toSearchText: () => '',
...
const rollup = {
  type: 'ROLLUP',
  isSortable: false,
  // Agregado (suma/conteo/media/mín/máx) sobre los registros relacionados que
  // apuntan a este vía una relación entrante. No se persiste: lo computa la
  // hidratación (settings.rollup). Solo lectura, como FORMULA.
  schema: (m) => nullable(z.number(), m),
  defaultValue: () => null,
  normalize: () => undefined, // nunca se guarda desde el cliente
  isReadOnly: true,
...
export const specialTypes = [array, rawJson, actor, position, relation, member, formula, rollup];
