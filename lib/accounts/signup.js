...
import { sendEmailVerification } from '@/lib/accounts/email-verification';
 ...
 * @param {boolean} [input.emailVerified] Por defecto `true`: solo el alta pública
 *   por email nace sin confirmar. OAuth ya ha demostrado el buzón, y los seeds,
 *   scripts y tests no deben arrastrar el bloqueo.
...
export async function createEmailAccount({ email, password }) {
  ...
  const session = await provisionAccount({
    ...
    // Único camino que nace sin confirmar: es el que puede usar el email de otro.
    emailVerified: false,
  });

  // El correo no bloquea el alta: si falla, la cuenta existe y hay reenvío.
  await sendEmailVerification({ userId: session.userId, email });
  return session;
}
