...
/** Tipos numéricos que un ROLLUP puede agregar. */
const ROLLUP_NUMERIC_TYPES = ['NUMBER', 'PERCENT', 'RATING', 'CURRENCY'];

/**
 * Fuentes posibles para un campo ROLLUP de un objeto: las relaciones ENTRANTES
 * (campos MANY_TO_ONE de otros objetos que apuntan a este) y, por cada una, los
 * campos numéricos del objeto de origen que se pueden agregar. Lo consume el
 * editor de campos para configurar un ROLLUP.
 * @param {import('@/lib/auth/permissions').Ctx} ctx
 * @param {{ objectMetadataId: string }} args
 */
export async function listRollupSources(ctx, { objectMetadataId }) {
  assertTenant(ctx);
  await connectToDatabase();
  ...
  for (const field of incoming) {
    const sourceObject = await ObjectMetadata.findById(field.objectMetadataId)
      .select('labelSingular labelPlural')
      .lean();
    if (!sourceObject) continue;

    const numericFields = await FieldMetadata.find({
      workspaceId: ctx.workspaceId,
      objectMetadataId: field.objectMetadataId,
      type: { $in: ROLLUP_NUMERIC_TYPES },
      deletedAt: null,
      isActive: true,
    })
      .select('name label')
    ...
    out.push({
      relationFieldId: String(field._id),
      relationFieldLabel: field.label,
      sourceObject: {
        labelSingular: sourceObject.labelSingular,
        labelPlural: sourceObject.labelPlural,
      },
      numericFields: numericFields.map((n) => ({ name: n.name, label: n.label })),
    });
  }
