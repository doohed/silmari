...
export function toActivityDTO(a, assigneeMap) {
  ...
  return {
    ...
    comm: a.comm
      ? {
          channel: a.comm.channel,
          direction: a.comm.direction,
          from: a.comm.from ?? '',
          to: (a.comm.to ?? []).slice(),
          provider: a.comm.provider ?? 'MANUAL',
          externalId: a.comm.externalId ?? null,
          occurredAt: a.comm.occurredAt ?? null,
        }
      : null,
...
/**
 * Registra una comunicación (email/WhatsApp) contra registros. Es la pieza de
 * dominio que usan `lib/email` y `lib/whatsapp`: sirve tanto para el registro
 * manual («escribí a X») como para el volcado desde un proveedor cuando exista.
 * @param {import('@/lib/auth/permissions').Ctx} ctx
 * @param {{ channel:'EMAIL'|'WHATSAPP', direction?:'INBOUND'|'OUTBOUND', subject?:string,
 *   body?:string, from?:string, to?:string|string[], provider?:string, externalId?:string,
 *   occurredAt?:string|Date, targets?:Array }} input
 */
export async function logCommunication(ctx, input) {
  assertTenant(ctx);
  await connectToDatabase();
  if (input.channel !== 'EMAIL' && input.channel !== 'WHATSAPP') {
    throw new ValidationError('Canal de comunicación no válido');
  }
 ...
 * @param {import('@/lib/auth/permissions').Ctx} ctx
 * @param {{ recordId: string, type?: 'NOTE'|'TASK'|'EMAIL'|'WHATSAPP' }} args
 */
