...
import { castConditionValue } from '@/lib/automations/condition-value';
...
function toAutomationDTO(a) {
  return {
    ...
    // Evaluaciones en las que las condiciones no casaron. Van aparte del log
    // (ver el modelo) y son la respuesta a «¿esta regla se está evaluando?».
    skippedCount: a.skippedCount ?? 0,
    lastSkippedAt: a.lastSkippedAt ?? null,
    runLog: (a.runLog ?? [])
      .slice()
      .reverse()
      .slice(0, 10)
...
async function validateBody(ctx, { name, trigger, conditions, actions }) {
  ...
  const normConditions = (conditions ?? []).map((c) => {
    ...
    // El valor se guarda **ya tipado**. Antes entraba tal cual desde un input de
    // texto y el `buildFilter` del tipo lo comparaba contra el valor real: una
    // condición sobre un SELECT o un BOOLEAN no casaba nunca, sin dar error.
    const cast = castConditionValue(c.value, field, c.operator);
    if (!cast.ok) throw new ValidationError(cast.error);
    return { fieldName: c.fieldName, operator: c.operator, value: cast.value };
  });
