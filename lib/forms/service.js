...
async function resolveAndValidate(ctx, { objectMetadataId, fields, dedupeFieldName }) {
  ...
  const normFields = (fields ?? []).map((f) => {
    ...
    if (!isWritableField(meta)) throw new ValidationError(`El campo "${f.fieldName}" no es editable`);
    return {
...
export async function updateForm(ctx, id, patch) {
  ...
  if (patch.redirectUrl !== undefined) form.redirectUrl = patch.redirectUrl?.trim() || null;
